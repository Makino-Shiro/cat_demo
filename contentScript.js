...
(function() {
    // 全域變數：記錄 meow 出現次數 / Global variable: record the number of meows
    let meowCount = 0;
    ...
    const frameInterval = 83; // 每83毫秒換一張圖（動畫幀速度） / Change image every 83 milliseconds (animation frame speed)

    // 連續播放動畫 / Play animation continuously
    setInterval(() => {
        cat.src = frames[currentFrame];
        currentFrame = (currentFrame + 1) % totalFrames;
    }, frameInterval);

    // 移動控制參數 / Movement control parameters
    let phase = "horizontal"; // "horizontal" 或 "vertical" / "horizontal" or "vertical"
    let moveTimer;            // 控制移動的計時器 / Timer for controlling movement
    let movesSinceStop = 0;   // 從上次暫停以來已移動的次數 / Number of moves since last pause
    let targetMoves = crtRandomNum(6, 8); // 隨機設定6~8次後觸發暫停 / Randomly set to trigger pause after 6~8 moves
    let touchMeActive = false; // 是否正處於「摸我」暫停狀態 / Whether the "touch me" pause is active
    let touchMeTimer = null;  // 控制暫停持續時間的計時器 / Timer for pause duration
    let activeTouchBubble = null; // 正在顯示的「touchme」對話泡 / Currently displayed "touchme" bubble
    let movespeed_1 = 20;
    let movespeed_2 = 50;
    
    // 重置小貓至起始位置（左下角） / Reset the cat to the starting position (bottom left)
    function resetCat() {
        if (moveTimer) {
    ...
    // 安排下一次移動（若已達暫停次數則觸發暫停） / Schedule the next move (if move count reaches target, trigger pause)
    ...
    // 水平移動：沿底部向右 / Horizontal movement: Move right along the bottom
    ...
    // 垂直移動：沿右側向上 / Vertical movement: Move upward along the right side
    ...
    // 終點效果：觸發後貓咪消失，效果持續8秒，再恢復原狀 / End effect: After triggering, the cat disappears, the effect lasts 8 seconds, then resets
    function triggerEndEffect() {
        ...
        if (effectChoice === 1) {
            ...
            // 螢幕藍色覆蓋，用於模擬藍屏 / Blue screen overlay to simulate a blue screen
            document.body.appendChild(blueScreen);
        } else if(effectChoice === 5){
            // 图片数组 / Array of images
            const images = [
            ...
            function changeImage() {
                ...
                `;
                index = (index + 1) % images.length; // 循环轮播 / Cycle through images
            }
            changeImage(); // 先执行一次 / Execute once first
            setInterval(changeImage, 2000); // 每 2 秒换一张 / Change image every 2 seconds
        }
    ...
    // 暫停功能：每移動6~8次後暫停，顯示「touchme.png」對話泡 / Pause functionality: After 6~8 moves, pause and display the "touchme.png" bubble
    function triggerTouchMePause() {
        ...
        // 點擊對話泡：若點擊則回到起點 / Click on the bubble: if clicked, return to the starting position
        ...
        // 10秒後若未點擊則自動取消暫停，恢復移動 / After 10 seconds, if not clicked, automatically cancel the pause and resume movement
    ...
    // 貓咪點擊事件： / Cat click event:
    // - 若處於「touchme」暫停狀態則回到起點 / If in "touchme" pause state, return to start
    // - 否則累加 meowCount；若達 3 次則觸發梗圖洪流，否則顯示「meow.png」對話泡0.5秒 / Otherwise, increment meowCount; if it reaches 3, trigger meme flood; else, display "meow.png" bubble for 0.5 seconds
    cat.addEventListener("click", (e) => {
        if (touchMeActive) {
            ...
            if (meowCount >= 3) {
                ...
                setTimeout(() => {
                    mogoBubble.remove();
                    triggerMemeFlood();   // 進入梗圖洪流 / Trigger meme flood
                    meowCount = 0;        // 重置 meow 次數 / Reset meow count
                }, 2000);
            } else {
    ...
    // 新增：觸發梗圖洪流 / Added: Trigger meme flood
    // 當 meow 點擊次數達 3 次後，此函式會依序每 0.5 秒產生一張 300x300 的 meme 圖片，
    // 總共 95 張，且位置隨機但不重疊，填滿整個頁面。 / When meow click count reaches 3, this function generates a 300x300 meme image every 0.5 seconds, for a total of 95 images, placed randomly without overlapping, filling the entire page.
    function triggerMemeFlood() {
        cat.style.display = "none";
        // 建立 1~95 的陣列，並轉成 meme 檔案 URL（meme1.jpg ~ meme95.jpg） / Create an array from 1 to 95 and convert to meme image URLs (meme1.jpg ~ meme95.jpg)
        let memeList = [];
        for (let i = 1; i <= 95; i++) {
            memeList.push(chrome.runtime.getURL("meme" + i + ".jpg"));
        }
        // 打亂陣列順序 / Shuffle the array
        memeList.sort(() => Math.random() - 0.5);
        
        // 儲存已放置區域，避免重疊 / Store placed areas to avoid overlap
        let placedRects = [];
        let index = 0;
        let interval = setInterval(() => {
            if (index >= memeList.length) {
                clearInterval(interval);
                // 所有 meme 放完後，等待 10 秒後清除並恢復初始狀態 / After all memes are placed, wait 10 seconds then remove them and reset to the initial state
                setTimeout(() => {
            ...
            // 尋找不重疊的位置 / Find a position that does not overlap
            let posFound = false;
            let attempts = 0;
            let left, top;
            let maxAttempts = 100;
            while (!posFound && attempts < maxAttempts) {
                ...
                // 檢查是否與已放置的區域重疊 / Check if it overlaps with any already placed area
            ...
            // 若經過多次嘗試仍未找到，則直接使用最後的值 / If a non-overlapping position isn't found after several attempts, use the last computed position
    ...
    // 視窗調整時，若小貓超出邊界則重置位置 / On window resize, if the cat goes out of bounds, reset its position
    ...
    // 初始化：設置小貓於左下角並開始移動 / Initialization: Place the cat at the bottom left and start moving
    resetCat();
})();
